...
void findCenterIndices(ChSystemGranularSMC &gran_sys, 
                          int numSpheres, 
...
int findMirroredParticleID(std::vector<int> left_index, 
                           std::vector<int> right_index,
                           int ID_left){
...
void printParticleInfo(ChSystemGranularSMC &gran_sys, 
    ...
    stream << numContacts << ", " << mySpherePos.x << ", " << mySpherePos.z << ", " << penetration << ", " << velo.x << ", " << velo.z <<", " << Fn.z << ", " << F_d.z << "\n";
}


// return number of contacts per particle
int findNumContacts(ChSystemGranularSMC &gran_sys, 
                    int numSpheres, 
                    int myParticleIndex,
                    float radius){
    int numContacts = 0;
