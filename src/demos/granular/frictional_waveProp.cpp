...
int main(int argc, char* argv[]) {
    ...
    std::vector<ChVector<float>> initialPos = initializeSpherePosition(x_dim_num, z_dim_num - 1, sphere_radius);
    //    std::vector<ChVector<float>> initialPos2 = initializePositions(x_dim_num, z_dim_num, sphere_radius);
    //    std::cout << " initialPos[0] = " << initialPos[0].x() << " " << initialPos[0].y() << " " << initialPos[0].z()
    //              << std::endl;
    //    std::cout << " initialPos[0] = " << initialPos2[0].x() << " " << initialPos2[0].y() << " " <<
    //    initialPos2[0].z()
    //              << std::endl;
    //
    //    std::cout << " initialPos.back() = " << initialPos.back().x() << " " << initialPos.back().y() << " "
    //              << initialPos.back().z() << std::endl;
    //    std::cout << " initialPos.back() = " << initialPos2.back().x() << " " << initialPos2.back().y() << " "
    //              << initialPos2.back().z() << std::endl;
    ...
    gran_sys.writeContactInfo(std::string(settling_contact_filename));
    ...
    while (curr_time < time_settling + time_Fduration && force_counter < F_ext_ratio_array_size) {
        ...
        if (force_counter % print_force_per_steps == 0) {
            pos_force_array = getSortedBoundaryForces(gran_sys, numSpheres, sphere_radius, 2 * kn, gamma_n, sphere_mass,
                                                      -box_Z / 2.0f);
    ...
    sprintf(outForceFile_string, "%s/ending_pos_force.csv", out_dir, F_ext_ratio);
    ...
    sprintf(end_contact_filename, "%s/ending_contact_forces", out_dir, F_ext_ratio);
    //    gran_sys.writeContactInfoFile(std::string(end_contact_filename));

    gran_sys.writeContactInfo(std::string(end_contact_filename));
